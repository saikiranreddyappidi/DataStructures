...
void print_wot_duplicates(struct node *start){
    start=start->next;
    while(start->next !=NULL){
...
void find(struct node *start){
    ...
    while(temp->next !=NULL){
       if(max!=temp->data && t<max){
            t=temp->data;
       }
        temp=temp->next;
    }
    printf("\n%d is the second largest element in the linked list",t);
}
...
void main(){
    ...
    printf("\nFinal linked list: ");		
    print(start);
    printf("\nFinal linked list without duplicates: ");		
    print_wot_duplicates(start);
    find(start);
}
